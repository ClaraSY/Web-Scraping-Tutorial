scrape_setup <- function(){
  my_url <<- 'https://www.glassdoor.com/'
  rd$navigate(my_url)
}

search_glassdoor <- function(job_title, location = 'Los Angeles, CA', page='home'){
  if(page == 'home'){
    search_xp <- '//*[@id="KeywordSearch"]'
    loc_xp <- '//*[@id="LocationSearch"]'
  } else {
    search_xp <-  '//*[@id="sc.keyword"]'
    loc_xp <- '//*[@id="sc.location"]'
  }
  search <- rd$findElement(using = 'xpath', search_xp)
  loc <- rd$findElement(using = 'xpath', loc_xp)
  search$clearElement()
  search$sendKeysToElement(sendKeys = list(job_title))
  
  loc$clearElement()
  loc$sendKeysToElement(sendKeys = list(location))
  loc$sendKeysToElement(sendKeys = list(key = 'enter'))
}
